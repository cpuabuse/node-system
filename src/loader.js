 ...
 */
async function initRecursion(rootDir, relativePath, initFilename, targetObject, extend) {
    ...
    // Initialize files
    for (var key in init) {
        let folder = relativePath, file = key, pathIsAbsolute = true, extend = false;
        ...
        await initRecursion(rootDir, pathIsAbsolute ? folder : path.join(relativePath, folder), file, targetObject[key] = {}, extend);
    }
}
/**
 ...
 */
async function initSettings(rootDir, relativeDir, filename // Filename, without extention; If null, then varname will be used instead
) {
    try {
